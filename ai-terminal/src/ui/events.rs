                                    ...
                                    < assistant_area.x + assistant_area.width
                            {
                                ...
                                if let Some(assistant_chunks) = app.assistant_area.map(|area| {
                                    ratatui::layout::Layout::default()
                                        ...
                                        .constraints([ratatui::layout::Constraint::Min(1), ratatui::layout::Constraint::Length(3)].as_ref())
                                        .split(area)
                                }) {
                                        ...
                                        for &(line_idx, ref cmd) in &app.extracted_commands {
                                            if line_idx as u16 == clicked_line {
                                                ...
                                                if let Some(line_content) = app.ai_output.get(line_idx) {
                                                    ...
                                                    let copy_button_start = line_content.find("[📋]").unwrap_or(0);
                                                    let execute_button_start = line_content.find("[▶️]").unwrap_or(0);
                                                    
                                                    // Calculate the actual screen positions
                                                    let copy_button_x_start = line_start_x + copy_button_start as u16;
                                                    let copy_button_x_end = copy_button_x_start + 4; // "[📋]" is 4 chars wide
                                                    
                                                    let execute_button_x_start = line_start_x + execute_button_start as u16;
                                                    let execute_button_x_end = execute_button_x_start + 4; // "[▶️]" is 4 chars wide
                                                    ...
                                                    if mouse_event.column >= copy_button_x_start && 
                                                       mouse_event.column < copy_button_x_end {
                                                        ...
                                                        app.copy_command_to_terminal(&command_to_copy);
                                                        break;
                                                    } 
                                                    // Check if click is on the execute button
                                                    else if mouse_event.column >= execute_button_x_start && 
                                                            mouse_event.column < execute_button_x_end {
                                                        // Execute button clicked
                                                        let command_to_execute = cmd.clone();
                                                        app.copy_and_execute_command(&command_to_execute);
                                                        break;
                                                    }
                                                }
                                            }

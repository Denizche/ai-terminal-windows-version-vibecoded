...
use crate::config::{
    ASSISTANT_TITLE, INPUT_TITLE, MAX_VISIBLE_SUGGESTIONS, SEPARATOR_LINE, SUGGESTIONS_TITLE,
    TERMINAL_TITLE,
};
...
fn draw_autocomplete_suggestions<B: Backend>(
    f: &mut Frame,
    app: &App,
    input_area: Rect,
    screen_size: Rect,
    ...
    let suggestions_widget = Paragraph::new(suggestions_text).block(
        Block::default()
            .borders(Borders::ALL)
            .border_type(BorderType::Rounded)
            .title(title),
